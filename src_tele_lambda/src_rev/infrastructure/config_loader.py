...
class ConfigLoader:
    ...
    def load(self) -> Tuple[InfiniteConfig, Dict[str, Any]]:
        ...
        try:
            ...
            # If it's a single dict (legacy support), wrap it in a list
            if isinstance(trading_conf, dict):
                trading_conf = [trading_conf]
            ...
            for conf in trading_conf:
                domain_configs.append(InfiniteConfig(
                    ...
                    star_adjustment_rate=Percentage(float(conf.get("star_adjustment_rate", 0.0)))
                ))
            ...
            return domain_configs, system_config
